"""
CDK stack for deploying text-archive MCP server to ECS behind ALB.
"""
...
class MCPStack(Stack):
    """
    CDK Stack for deploying text-archive MCP server to ECS with ALB.
    ...
    def _create_task_definition(self) -> ecs.FargateTaskDefinition:
        ...
        # Load secrets from AWS Secrets Manager using exact ARN
        skill_secret = secretsmanager.Secret.from_secret_complete_arn(
            self,
            "SkillSecret",
        ...
        container = task_def.add_container(
            ...
            secrets={  # Secrets from AWS Secrets Manager (injected at runtime)
                "REUTERS_ARCHIVE_CLIENT_ID": ecs.Secret.from_secrets_manager(
                    skill_secret, "REUTERS_ARCHIVE_CLIENT_ID"
                ),
                "REUTERS_ARCHIVE_CLIENT_SECRET": ecs.Secret.from_secrets_manager(
                    skill_secret, "REUTERS_ARCHIVE_CLIENT_SECRET"
                ),
            },
        )
    ...
    def _create_load_balancer(self) -> elbv2.ApplicationLoadBalancer:
        ...
        alb = elbv2.ApplicationLoadBalancer(
            ...
            idle_timeout=Duration.seconds(300),  # 5 minutes for long-running archive queries
        )
        return alb
