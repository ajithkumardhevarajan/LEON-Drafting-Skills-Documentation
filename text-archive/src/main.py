"""
Text Archive MCP Server

This is the main FastAPI server implementing the Model Context Protocol (MCP)
for Reuters Text Archive Q&A capabilities. It handles JSON-RPC 2.0 requests from
the backend and routes them to appropriate handlers.
...
class TextArchiveMCPServer:
    ...
    def __init__(self):
        self.app = FastAPI(
            title="Text Archive MCP Server",
            description="Model Context Protocol server for Reuters Text Archive Q&A capabilities",
            version="0.1.0",
        )
        ...
        logger.info("Text Archive MCP Server initialized")
    ...
    async def _handle_initialize(
        ...
        response = InitializeResponse(
            ...
            capabilities=ServerCapabilities(
                tools={},  # We support text archive tools
                resources={},  # No resources in this server
                prompts={},  # No prompts in this server
            ),
            serverInfo=ServerInfo(name="Text Archive MCP", version="0.1.0"),
        )
...
def main():
    ...
    # Default to 8000 for Docker/production, override with MCP_PORT env var (e.g., 8005 for local dev)
    port = int(os.getenv("MCP_PORT", "8000"))

    # Create server instance
    server = TextArchiveMCPServer()

    # Log startup
    logger.info(f"Starting Text Archive MCP Server on {host}:{port}")
